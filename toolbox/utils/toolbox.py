...
def runStats() -> str:
    ...
    remote_shard_0 = [
        ...
        "blockchain",
        "latest-headers",
        f"--node=https://api.s0.{ourNetwork}.hmny.io",
    ]
    result_remote_shard_0 = run(remote_shard_0, stdout=PIPE, stderr=PIPE, universal_newlines=True)
    remote_data_shard_0 = json.loads(result_remote_shard_0.stdout)
    remote_shard = [
        ...
        "blockchain",
        "latest-headers",
        f"--node=https://api.s{ourShard}.{ourNetwork}.hmny.io",
    ]
    try:
        result_remote_shard = run(remote_shard, stdout=PIPE, stderr=PIPE, universal_newlines=True)
        remote_data_shard = json.loads(result_remote_shard.stdout)
        ...
        *
        ...
        {stringStars()}
        input(
            f"* No remote data returned, RPC may be unresponsive. Wait few moments and try again. Press ENTER to return to the main menu."
        )
        {stringStars()}
        return
    local_shard = [f"{validatorToolbox.hmyAppPath}", "blockchain", "latest-headers"]
    try:
        ...
        *
        ...
        {stringStars()}
        input(
            f"* No local data detected, did you just restart Harmony? Wait a few moments and try again. Press ENTER to return to the main menu."
        )
        {stringStars()}
        return
    if environ.get("SHARD") == "0":
